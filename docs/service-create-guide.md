...
> **TL;DR** — 템플릿 골라서 새 `lab-{이름}` repo 만들고, Netlify에 올리고, QA / privacy 체크리스트 통과하면 메인서비스 `serviceLinks`에 추가 요청.
...
> **코드 / 스택 / 트리거 / RLS / 클라이언트 셋업** 같은 구현 상세는 이 가이드에서 다루지 않습니다. **Playground 엔지니어링 가이드**(예: "Supabase 스키마 격리", "익명 가입 + RLS 격리", "Netlify 브랜치 미리보기 워크플로우")를 별도로 참고하세요. 위치는 [README의 엔지니어링 상세 가이드 섹션](../README.md#엔지니어링-상세-가이드).
